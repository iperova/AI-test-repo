...
def run():
    ...
    pipeline_options = PipelineOptions(
        ...
        project=PROJECT_ID,
        job_name = 'phoenix-aggregation-pipeline',
        temp_location='gs://kw-ds-vertex-ai-test/temp',
        staging_location='gs://kw-ds-vertex-ai-test/staging',
        template_location='gs://kw-ds-vertex-ai-test/templates/phoenix_aggregation_template',
    ...
    with beam.Pipeline(options=pipeline_options) as pipeline:
        ...
        results = []
        for housing_type in ['houses', 'condos']:
            ...
            QUERY_TEMPLATE = f"""
            CREATE OR REPLACE TABLE `kw-data-science-playgorund.avm_us_{housing_type}_new_source.phoenix_2019` AS
            WITH 
            t_listings_origin AS
            (
            SELECT
              *,
              ST_GEOGFROMTEXT('POINT(' || property_address.coordinates_gp.longitude || ' ' || property_address.coordinates_gp.latitude || ')') AS list_geo
            ...
            WHERE 
              ...
              AND total_bathroom_count < 10
              AND total_bedroom_count < 10
              AND year_built > 1800
              AND LOWER(listing_status) IN ('sold', 'off market')
              AND property_address.coordinates_gp.latitude BETWEEN -90 AND 90
            ),
            
            t_metro_areas AS
            (
              SELECT name AS metropolitan_area_name, cbsa, ST_GeogFromText(wkt) AS metro_polygon
            ...
            (
              SELECT 
                * EXCEPT(metro_polygon)
              FROM 
                t_listings_origin, t_metro_areas 
              WHERE ST_INTERSECTS(t_listings_origin.list_geo, t_metro_areas.metro_polygon)
            ),
            ...
            FROM 
              t_metro_areas_data AS l, UNNEST(virtual_tours) vt
            WHERE TRUE
              AND living_area > 0
            ...
            (
              ...
              FROM 
                t_raw_listings
              WHERE DATE_DIFF(close_dt, listing_dt, day)>0 
            ...
            FROM 
              t_metro_areas_data AS l, UNNEST(photo_data)           
            GROUP BY 1
            ),
            ...
            CREATE OR REPLACE TABLE `kw-data-science-playgorund.avm_us_{housing_type}_new_source.phoenix_2019_bg` AS
            WITH
            t_raw_data  AS 
            (
              SELECT 
                * EXCEPT(listing_description, virtual_tour_url, roof_types, utilities, water_source, parking_features, 
                construction_materials, flooring, day_on_market),
                ST_GEOGFROMTEXT('POINT(' || lon || ' ' || lat || ')') AS list_geo
              FROM `kw-data-science-playgorund.avm_us_{housing_type}_new_source.phoenix_2019`
            ...
            CREATE OR REPLACE TABLE `kw-data-science-playgorund.avm_us_{housing_type}_new_source.phoenix_2019_bg_prec` AS 
            WITH 
            t_precisely AS 
            (
              SELECT
            ...
            (
              ...
              FROM 
                `kw-data-science-playgorund.avm_us_{housing_type}_new_source.phoenix_2019_bg` AS list
            ...
            WHERE TRUE
              ...
              AND population_grow < 7500
              AND population_grow > -1000
              AND population_estimate < 7000
              AND population_estimate > -750;
            
            
            
            CREATE OR REPLACE TABLE `kw-data-science-playgorund.avm_us_{housing_type}_new_source.phoenix_2019_bg_prec_econ` AS
            ...
            (
              ...
              l.state_prov_short
              FROM `kw-data-science-playgorund.avm_us_{housing_type}_new_source.phoenix_2019_bg_prec` AS l
            ...
            CREATE OR REPLACE TABLE `kw-data-science-playgorund.avm_us_{housing_type}_new_source.schools_phoenix_2019`
            AS
            WITH t_school AS
            (
            SELECT 
            ...
            FROM `kw-data-science-playgorund.avm_us_{housing_type}_new_source.phoenix_2019_bg_prec_econ` AS l, t_school
            ...
            FROM `kw-data-science-playgorund.avm_us_{housing_type}_new_source.phoenix_2019_bg_prec_econ` AS l
            ...
            CREATE OR REPLACE TABLE `kw-data-science-playgorund.avm_us_{housing_type}_new_source.phoenix_2019_bg_prec_econ_school` AS
            
            SELECT *
            FROM `kw-data-science-playgorund.avm_us_{housing_type}_new_source.phoenix_2019_bg_prec_econ`
                INNER JOIN `kw-data-science-playgorund.avm_us_{housing_type}_new_source.schools_phoenix_2019` USING (listing_id);
            
            
            CREATE OR REPLACE TABLE `kw-data-science-playgorund.avm_us_{housing_type}_new_source.phoenix_2019_prophet_raw`  AS
            ...
            FROM `kw-data-science-playgorund.avm_us_{housing_type}_new_source.phoenix_2019_bg_prec_econ_school`
            WHERE TRUE
                AND city_long IS NOT NULL
                AND state_prov_short IS NOT NULL
                AND living_area  IS NOT NULL
                AND total_bedroom_count  IS NOT NULL
                AND total_bathroom_count  IS NOT NULL
            ...
            CREATE OR REPLACE TABLE `kw-data-science-playgorund.avm_us_{housing_type}_new_source.phoenix_2019_prophet`  AS
            
            SELECT
                * EXCEPT(city_long, address, mls_id, mls_number,listing_status, postal_code, postal_code_short, 
                state_prov_short, current_listing_price)
            FROM `kw-data-science-playgorund.avm_us_{housing_type}_new_source.phoenix_2019_prophet_raw`;
             
             
            CREATE OR REPLACE TABLE `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.phoenix` AS
            WITH 
            t_listings_origin AS
            (
            SELECT
            ...
            CREATE OR REPLACE TABLE `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.phoenix_bg` AS
            WITH
            t_raw_data AS 
            (
              SELECT 
                * EXCEPT(listing_description, virtual_tour_url, roof_types, utilities, water_source, parking_features, construction_materials, flooring),
                ST_GEOGFROMTEXT('POINT(' || lon || ' ' || lat || ')') AS list_geo
              FROM `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.phoenix`
              WHERE lat BETWEEN -90 AND 90
              ORDER BY listing_id
            ),
            ...
            CREATE OR REPLACE TABLE `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.phoenix_bg_prec` AS
            WITH 
            t_precisely AS 
            (
              SELECT
            ...
            (
              SELECT
                * EXCEPT (code, latitude, longitude, landarea, waterarea, population_density, 
                ...
                contract_rent_percent_median, household_income_median, home_value_median)
              FROM 
                `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.phoenix_bg` AS list
                  LEFT JOIN t_precisely AS pr ON list.GEOID = pr.code
              WHERE pr.code IS NULL
            ),
            ...
            CREATE OR REPLACE TABLE `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.phoenix_bg_prec_econ` AS
            ...
            (
              ...
              original_listing_category, property_type),
              l.state_prov_short
              FROM `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.phoenix_bg_prec` AS l
              LEFT JOIN `kw-data-science-playgorund.price_prediction.state_population` USING (state_prov_short)
                LEFT JOIN `kw-data-science-playgorund.price_prediction.temperature_tornado` USING (state_prov_short)
                  LEFT JOIN `kw-data-science-playgorund.price_prediction.mortgage_not_seasonality_upd` AS m USING (close_year, num_week)
                    LEFT JOIN `kw-data-science-playgorund.price_prediction.case_shiller_hpi_not_seasonality_upd` AS cs 
            ...
            CREATE OR REPLACE TABLE `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.schools_phoenix`
            ...
            SELECT 
                ...
                t_school.pctile_y1,
                ST_DISTANCE(list_geo, lat_lon) AS distance
            FROM `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.phoenix_bg_prec_econ` AS l, t_school
            WHERE ST_INTERSECTS(l.list_geo, t_school.geom)
            ),
            ...
            SELECT 
                ...
                l.city_long,
                t_elementary.distance
            FROM `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.phoenix_bg_prec_econ` AS l
                LEFT JOIN t_elementary USING (listing_id)
            WHERE t_elementary.distance IS NULL
            ),
            ...
            CREATE OR REPLACE TABLE `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.phoenix_bg_prec_econ_school` AS
            
            SELECT *
            FROM `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.phoenix_bg_prec_econ`
                INNER JOIN `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.schools_phoenix` USING (listing_id);
            
            
            CREATE OR REPLACE TABLE `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.phoenix_prophet_raw`  AS
            WITH
            t_listings AS 
            (
            SELECT * EXCEPT (listing_dt, property_subtype, list_geo, polygon, dma_name,  original_listing_price, lot_size_area),
            ...
            FROM `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.phoenix_bg_prec_econ_school`
            WHERE TRUE
                AND city_long IS NOT NULL
                AND state_prov_short IS NOT NULL
                AND living_area  IS NOT NULL
            ...
            CREATE OR REPLACE TABLE `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.phoenix_prophet`  AS
            ...
            (
              ...
              FROM `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.phoenix_prophet_raw`
            ...
            CREATE OR REPLACE TABLE `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.{housing_type}_phoenix_last`  AS
            
              SELECT * EXCEPT(GEOID, city_long, address, mls_id, mls_number, 
                    postal_code, postal_code_short, state_prov_short, current_listing_price, sa_mls_id)
              FROM `kw-data-science-playgorund.avm_us_{housing_type}_not_sold_new_source.phoenix_prophet`;
        """
...
# python phoenix_aggregation_dataflow.py --worker_machine_type=e2-standard-4
...
#     -d '{
#         "jobName": "phoenix-aggregation-pipeline",
#         "parameters": {},
#         "environment": {
...
#         }
#     }' \
#     "https://dataflow.googleapis.com/v1b3/projects/kw-data-science-playgorund/locations/us-east1/templates:launch?gcsPath=gs://kw-ds-vertex-ai-test/templates/phoenix_aggregation_template"
