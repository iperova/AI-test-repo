...
class DRPredictModelDoFn(beam.DoFn):
    ...
    def process(self, df_prophet):
        ...
        def predict_by_project(PROJECT_NAME, MONTH):
        ...
        predict_by_project(self.project_name, self.month)

        # return logging.info(f"{self.project_name} prediction starts!!!")
...
def run():
    ...
    with beam.Pipeline(options=pipeline_options) as pipeline:
        ...
        for PROJECT_NAME in project_list:
            results.append(
                pipeline
            | f"Process {PROJECT_NAME}" >> ProcessDataset(project_name=PROJECT_NAME, month=MONTH,
